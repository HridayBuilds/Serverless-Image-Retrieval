...
variable "rekognition_index_max_concurrency" {
  description = "IndexPhotos Distributed Map's MaxConcurrency. This account's measured IndexFaces TPS quota is 5"
  type        = number
  default     = 5
}

variable "rekognition_search_max_concurrency" {
  description = "MatchAttendees Distributed Map's MaxConcurrency, for SearchFacesByImage."
  type        = number
  default     = 5
}
